...
def autoaug_batch_process_map_reset_fn(images, labels):
  ...
  if FLAGS.aug_type == 'autoaug':
    aa_images = tf.py_func(pool_policy_augmentation, [tf.identity(images)],
                           [tf.float32])
    aa_images_2 = tf.py_func(pool_policy_augmentation, [tf.identity(images)],
                           [tf.float32])
    
  elif FLAGS.aug_type == 'randaug':
    aa_images = tf.py_func(random_augmentation, [tf.identity(images)],
                           [tf.float32])
    aa_images_2 = tf.py_func(random_augmentation, [tf.identity(images)],
                           [tf.float32])
    
  elif FLAGS.aug_type == 'default':
    aa_images = tf.py_func(cifar_process, [tf.identity(images)], [tf.float32])
    aa_images_2 = tf.py_func(cifar_process, [tf.identity(images)], [tf.float32])
    
  else:
    raise NotImplementedError('{} aug_type does not exist'.format(
        FLAGS.aug_type))
  aa_images = tf.reshape(aa_images, [-1] + images.shape.as_list()[1:])
  aa_images_2 = tf.reshape(aa_images_2, [-1] + images.shape.as_list()[1:])
  images = tf.concat([tf.expand_dims(images, 1),
                      tf.expand_dims(aa_images, 1),tf.expand_dims(aa_images_2, 1)],
                     axis=1)
